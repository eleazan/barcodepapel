<x-layouts.admin title="Nuevo pedido">
        ...
        <x-admin.card>
            <form method="POST" action="{{ route('admin.orders.store') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="customer_name" id="customer_name" value="{{ old('customer_name') }}" class="form-input" required>
                            @error('customer_name') <p class="form-error">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label for="customer_phone" class="form-label">Teléfono</label>
                            <input type="text" name="customer_phone" id="customer_phone" value="{{ old('customer_phone') }}" class="form-input" required>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input type="email" name="customer_email" id="customer_email" value="{{ old('customer_email') }}" class="form-input">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="sm:col-span-2">
                            ...
                            <textarea name="delivery_address" id="delivery_address" rows="2" class="form-textarea" required>{{ old('delivery_address') }}</textarea>
                        ...
                        <div>
                            ...
                            <input type="number" name="delivery_fee" id="delivery_fee" value="{{ old('delivery_fee', 0) }}" class="form-input" step="0.01" min="0" required x-model="deliveryFee">
                                ...
                                <div class="flex-1">
                                    <select :name="`items[${index}][product_id]`" class="form-select text-sm" required x-model="item.product_id" @change="updatePrice(index)">
                                        ...
                                        @foreach ($products as $product)
                                            <option value="{{ $product->id }}" data-price="{{ $product->price }}">{{ $product->name }} — {{ number_format((float) $product->price, 2, ',', '.') }} € ({{ $product->stock }} disp.)</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="w-24">
                                    <input type="number" :name="`items[${index}][quantity]`" x-model.number="item.quantity" min="1" class="form-input text-sm text-center" placeholder="Cant." required @input="recalculate()">
                                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label for="notes" class="form-label">Notas <span class="text-gray-400 font-normal">(opcional)</span></label>
                ...
                <div class="flex items-center gap-3 pt-2">
                    ...
                    <a href="{{ route('admin.orders.index') }}" class="btn-secondary">Cancelar</a>
                </div>
            </form>
        </x-admin.card>
    </div>
